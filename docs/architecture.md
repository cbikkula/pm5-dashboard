...
The pure analysis layer (sanitizers, curve/shape math, baseline engine, cue governor, Race Lab, power profile, and the v1.21 stroke-curve codec/retention/evidence engines — all DOM-free) lives in `pm5web/analysis.js`, loaded as a classic script before the main inline script. v1.21 adds a second module, `pm5web/curves.js`: the stroke-level-evidence layer (IndexedDB curve-detail store, in-memory capture append, and the replay Stroke Evidence UI), loaded between analysis.js and the main script. v1.22 adds `pm5web/insights.js`: the deterministic cross-session Insights engine (pure cohort/aggregation/finding functions) plus the Insights page renderer and dependency-free canvas charts, loaded after curves.js. v1.23 adds `pm5web/transport.js`: the BLE connection state machine (13 states, explicit legal-transition table, generation tokens against stale async completions), the status-family packet-liveness watchdog (stale after 5 s of validated-status silence, monotonic clock, completed-workout disarm, 10 s warn cooldown, bounded gap recorder), the reconnect continuity rule (same workout only when elapsed/distance/stroke counters have not regressed; otherwise the interrupted session is finalized separately), the packet minimum-length table, privacy-safe diagnostics (slug-vocabulary events, bounded buffer, export re-sanitized — never device identifiers, packet bytes, or workout data), and the deterministic transport simulator that feeds byte-accurate notifications through the production handlers. All three are served network-first by the service worker so a page can never mix releases, and all are cached for offline. The size guard in `tests/run.js` measures the total offline app (index.html + analysis.js + curves.js + insights.js + sw.js) and still caps index.html at 660 KB. The total limit moved in v1.22.0 from 768 KB to 832 KB and in v1.23.0 to **860 KB** (transport.js ~20 KB after ~2.8 KB measured cleanup; permitted ceiling was 864): Insights adds ~55 KB of engine + page, measured cleanup recovered ~3 KB of stale comments/markup first, and a responsible implementation could not fit the remainder — documented here and in CHANGELOG.md, enforced in CI, every asset measured, no unmeasured code paths. `tests/harness.js` concatenates the four scripts into one vm sandbox; `scripts/syntax-check.js` parses each. Formulas: `docs/analysis-methods.md`.
