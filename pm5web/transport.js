/* RowTracer — transport module (v1.23.0): the authoritative BLE
...
function createDiagnostics(opts) {
  ...
  const D = {
    ...
    exportSafe(extra) {
      return {
        kind: "pm5-connection-diagnostics", producer: "RowTracer",
...
function exportConnDiag() {
  ...
  triggerDownload(blob, `rowtracer-connection-diagnostics-${new Date().toISOString().slice(0, 10)}.json`);
