...
| 34 | ✅ done v1.16.0 | Visual theme refresh + UI polish       | Premium depth/motion pass under the bundle guard: theme-safe button hover/press (fixed a cross-theme hardcoded-colour bug), frosted modal backdrop + elevation, menu-card hover lift, header gradient, themed scrollbars. No behavior or feature changes; all 8 themes + live monitor + Performance + Replay verified intact at desktop and 375px. |
| 35 | ✅ done v1.17.0 | Live Monitor 2.0 / race-display polish | Training-screen refinement: tabular live numbers (cards + race mode) so digits don't jitter, stronger primary-card hierarchy + depth, theme-safe target-zone tints, gradient/glow workout progress bar. **Mobile fix:** the fixed 3-column monitor grid had no phone breakpoint and overflowed below ~520px — now collapses to a single scrollable column at ≤600px. CSS-only; no BLE/metric-calc changes. Verified in Demo Mode at desktop + 375px. |
